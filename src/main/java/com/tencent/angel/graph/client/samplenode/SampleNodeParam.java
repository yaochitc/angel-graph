...
public class SampleNodeParam extends GetParam {
	...
	public SampleNodeParam(int matrixId, int nodeType, int count, Map<Integer, Integer> countPerPart) {
		super(matrixId);
	...
	public List<PartitionGetParam> split() {
		...
		for (PartitionKey partitionKey : partitions) {
			...
			if (count > 0) {
				partParams.add(new PartSampleNodeParam(matrixId, partitionKey, nodeType, count));
			}
		}
