package com.tencent.angel.algorithm.nn.keras
...
object ReusableLayer {
  def apply[T: ClassTag](delegate: KerasLayer[Tensor[T], Tensor[T], T],
                         hasGradInput: Boolean = true)
                        (implicit ev: TensorNumeric[T]): ReusableLayer[T] = {
    new ReusableLayer[T](delegate, hasGradInput)
  }
